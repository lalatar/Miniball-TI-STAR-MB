...
#include "MiniBallTextGenerator.hh"
    ...
    file_written(false)
{
  ...
  hist_file.open(file_name.c_str(), std::ofstream::out);

  if(!hist_file) {
	  std::cerr << "MiniBallTextHistogram: "
	 << "could not open histogram file " << file_name << std::endl;
    return;
  }
  ...
  if(!data) {
	  std::cerr << "MiniBallTextHistogram: "
         << "could not allocate memory for histogram" << std::endl;
    return;
  }
...
MiniBallTextHistogram::~MiniBallTextHistogram()
{
  if(data && hist_file) {
    ...
    hist_file << "error: not enough memory" << std::endl;
    hist_file.close();
  } else if(data)
    free(data);
}
...
void MiniBallTextHistogram::Close()
{
  if(hist_file && !file_written) {
    ...
    for(G4int i=0; i<bin; i++) 
       hist_file << low+((G4double)i)*bin_size << " " << data[i] << std::endl;
    hist_file.close();
    file_written=true;
  }
}
void MiniBallTextHistogram::Fill(G4double value)
{
  ...
  if(value_bin<0 || value_bin>=bin) {
	  std::cerr << "MiniBallTextHistogram: unexpected error" << std::endl;
    return;
  }
  ...
  : started_writing(false)
{
  ...
  tuple_file.open(file_name.c_str(), std::ofstream::out);

  if(!tuple_file) {
	  std::cerr << "MiniBallTextHistogram: "
	 << "could not open tuple file " << file_name << std::endl;
    return;
  }
}
...
{
  if(started_writing) {
	  std::cerr << "MiniBallTextTuple: cannot create new column after "
